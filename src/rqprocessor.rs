...
impl RaptorQProcessor {
    ...
    fn get_encoder(&self, path: &Path) -> Result<(Encoder, u32), RqProcessorError> {
        ...
        let mut buffer = vec![0; ORIGINAL_FILE_LOAD_CHUNK_SIZE];
        // Create a buffer to hold the entire file data for encoder creation
        let mut data = Vec::with_capacity(source_size as usize);
        // Read the file data in chunks
        loop {
            let bytes_read = file.read(&mut buffer).map_err(|err| RqProcessorError::new_file_err("get_encoder", "Cannot read input file", path, err.to_string()))?;
            if bytes_read == 0 {
                break;
            }
